...
| G14 Stripe API 90% Program    | In progress    | `docs/STRIPE_COMPATIBILITY_90_TARGET.md` defines 90% L1+ target, current 57/587 baseline, family thresholds, and chunk plan                              |
...
- OpenAPI operation baseline: `57 / 587`, `9.7%`
...
Verified on 2026-05-11 from branch `codex/billing-matrix-simulation-support`:

- `go test ./... -count=1`
- `go run ./cmd/billtap compatibility scorecard --output-dir /tmp/billtap-compat-scorecard-billing-matrix-final3`
  - result: `imported=49 skipped=1 unsupported=1 mismatch=0 error=0`
- `go run ./cmd/billtap compatibility inventory --openapi /tmp/stripe-openapi-spec3.json --output-dir /tmp/billtap-inventory-full-final3 --source stripe-openapi-master`
  - result: `operations=587 implemented=57 inventory_only=530 schema_validated=587 implemented_percent=9.7`
- `npm run build`
