...
| Payment methods         | `GET /v1/payment_methods?customer={id}&type=card`, `GET /v1/customers/{id}/payment_methods?type=card`                                                              | Partial          | Returns a deterministic sandbox card projection for a known customer, including a saved portal default payment method when one was simulated. Customer fixtures can opt into empty or explicit payment-method lists for no-card and multi-card local scenarios. Create, attach, detach, and general update are not supported.                                                                                                                                             |
