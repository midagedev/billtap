...
func DefaultClaims() []Claim {
	...
	add(http.MethodPost, "/v1/invoices/create_preview", Claim{Level: "L3", Stateful: true, Risks: []string{"local subscription-update proration with default_tax_rates/automatic_tax matching confirmed invoices; pending invoice items and full multi-item preview parity are not modeled"}})
	add(http.MethodGet, "/v1/invoices/upcoming", Claim{Level: "L3", Stateful: true, Risks: []string{"Stripe-compatible upcoming preview alias backed by the same local proration and tax subset"}})
	add(http.MethodPost, "/v1/invoices/upcoming", Claim{Level: "L3", Stateful: true, Risks: []string{"local form-compatible upcoming preview convenience with the same tax subset as create_preview"}})
