...
func DefaultClaims() []Claim {
	...
	add(http.MethodDelete, "/v1/products/{id}", Claim{Level: "L3", Stateful: true, Risks: []string{"removes local product evidence; existing prices keep referencing the deleted product id"}})
	...
	add(http.MethodPost, "/v1/checkout/sessions/{id}/expire", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"checkout.session.expired"}, Risks: []string{"expires open sessions only; non-open sessions are rejected and payment recovery is not modeled"}})
	add(http.MethodGet, "/v1/checkout/sessions/{id}/line_items", Claim{Level: "L3", Stateful: true, Risks: []string{"line amounts are pre-discount; per-line discount and tax splits are not modeled"}})
	add(http.MethodPost, "/v1/checkout/sessions/{id}", Claim{Level: "L3", Stateful: true, Risks: []string{"open sessions only; metadata merge plus line_items[N][quantity] overrides with immutable prices; other Stripe update params are rejected"}})
	...
	portalConfigurationRisk := []string{"local portal configuration evidence only; the hosted portal does not render configuration features"}
	add(http.MethodGet, "/v1/billing_portal/configurations", Claim{Level: "L2", Stateful: true, Risks: portalConfigurationRisk})
	add(http.MethodPost, "/v1/billing_portal/configurations", Claim{Level: "L3", Stateful: true, Risks: portalConfigurationRisk})
	add(http.MethodGet, "/v1/billing_portal/configurations/{id}", Claim{Level: "L2", Stateful: true, Risks: portalConfigurationRisk})
	add(http.MethodPost, "/v1/billing_portal/configurations/{id}", Claim{Level: "L3", Stateful: true, Risks: portalConfigurationRisk})
	...
	add(http.MethodGet, "/v1/customers/{id}/discount", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.discount.deleted"}, Risks: []string{"bounded single-discount subset"}})
	add(http.MethodDelete, "/v1/customers/{id}/discount", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.discount.deleted"}, Risks: []string{"bounded single-discount subset"}})
	add(http.MethodGet, "/v1/subscriptions/{id}/discount", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.discount.deleted"}, Risks: []string{"bounded single-discount subset"}})
	add(http.MethodDelete, "/v1/subscriptions/{id}/discount", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.discount.deleted", "customer.subscription.updated"}, Risks: []string{"bounded single-discount subset"}})
	add(http.MethodGet, "/v1/customers/{id}/subscriptions", Claim{Level: "L3", Stateful: true, Risks: []string{"legacy nested subscription route backed by the same local subscription graph and status filters as /v1/subscriptions"}})
	add(http.MethodPost, "/v1/customers/{id}/subscriptions", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.subscription.created", "invoice.paid", "payment_intent.succeeded"}})
	add(http.MethodGet, "/v1/customers/{id}/subscriptions/{id}", Claim{Level: "L3", Stateful: true})
	...
	add(http.MethodGet, "/v1/customers/{id}/subscriptions/{id}/discount", Claim{Level: "L3", Stateful: true, Risks: []string{"returns the subscription's single local discount only"}})
	add(http.MethodDelete, "/v1/customers/{id}/subscriptions/{id}/discount", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.discount.deleted", "customer.subscription.updated"}, Risks: []string{"clears the subscription's single local discount only"}})

	add(http.MethodPost, "/v1/subscription_items", Claim{Level: "L3", Stateful: true, ScorecardCases: []string{"subscription_items.create.invalid_quantity"}, SDKSmoke: []string{"stripe-node"}, Risks: []string{"proration_behavior always_invoice/create_prorations reuses subscription-update proration (subscription default_tax_rates only; item tax_rates evidence-only)", "item IDs keep the si_<subscription>_<n> shape but are stored at creation, so deletes no longer shift later item IDs; subscriptions stored before that change are backfilled with their position-derived IDs on next write"}})
	add(http.MethodGet, "/v1/subscription_items", Claim{Level: "L3", Stateful: true, Risks: []string{"lists resolved items across subscriptions; supports the subscription filter and standard list envelope only"}})
	add(http.MethodGet, "/v1/subscription_items/{id}", Claim{Level: "L3", Stateful: true})
	add(http.MethodPost, "/v1/subscription_items/{id}", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"customer.subscription.updated"}, Risks: []string{"price/quantity replacement with the same proration subset as item create (always_invoice/create_prorations); payment_behavior and discounts are not modeled", "item tax_rates are evidence on the response only; totals use subscription default_tax_rates"}})
	add(http.MethodDelete, "/v1/subscription_items/{id}", Claim{Level: "L3", Stateful: true, SDKSmoke: []string{"stripe-node"}, Risks: []string{"proration_behavior supported; last item delete rejected; clear_usage evidence-only (metered usage not modeled)", "stored item IDs stay stable after a middle-item delete; a later add reuses the lowest unused index"}})
	...
	add(http.MethodPost, "/v1/invoices/{id}/void", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"invoice.voided"}, Risks: []string{"moves open invoices to void with local evidence only; unvoiding is not modeled"}})
	add(http.MethodPost, "/v1/invoices/{id}/mark_uncollectible", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"invoice.marked_uncollectible"}, Risks: []string{"moves open invoices to uncollectible with local evidence only; reopening is not modeled"}})
	add(http.MethodPost, "/v1/invoices/{id}", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; bounded subset (description, days_until_due, default_payment_method, metadata merge)"}})
	add(http.MethodDelete, "/v1/invoices/{id}", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; removes attached lines and timeline evidence with the invoice"}})
	add(http.MethodPost, "/v1/invoices/{id}/add_lines", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; amount/description/currency lines through the invoice-item path; price_data and tax lines are not modeled"}})
	add(http.MethodPost, "/v1/invoices/{id}/update_lines", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; amount/description updates recompute subtotal/total/amount_due"}})
	add(http.MethodPost, "/v1/invoices/{id}/remove_lines", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; removing every line leaves a zero-amount draft"}})
	add(http.MethodPost, "/v1/invoices/{id}/lines/{line_item_id}", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; amount/description/metadata updates recompute subtotal/total/amount_due; pricing and tax line edits are not modeled"}})
	add(http.MethodPost, "/v1/invoices/{id}/attach_payment", Claim{Level: "L3", Stateful: true, Risks: []string{"draft invoices only; records the attached PaymentIntent or payment record as invoice evidence; collection still runs through finalize/pay"}})
	add(http.MethodPost, "/v1/subscriptions/{id}/migrate", Claim{Level: "L3", Stateful: true, Risks: []string{"records billing_mode=flexible as subscription metadata evidence; flexible-billing proration recalculation is not modeled"}})
	...
	add(http.MethodGet, "/v1/invoices/upcoming", Claim{Level: "L3", Stateful: true, Risks: []string{"Stripe-compatible upcoming: no-override subscription preview returns next-period invoice; item overrides share create_preview proration/tax subset"}})
	add(http.MethodPost, "/v1/invoices/upcoming", Claim{Level: "L3", Stateful: true, Risks: []string{"local form-compatible upcoming preview convenience with the same next-period and proration tax subset as create_preview"}})
	add(http.MethodGet, "/v1/invoices/search", Claim{Level: "L3", Stateful: true, Risks: []string{"supports a measured invoice search subset for id, customer, subscription, and status clauses joined by AND"}})
	add(http.MethodGet, "/v1/invoiceitems", Claim{Level: "L2", Stateful: true, Risks: []string{"manual one-time invoice item evidence only"}})
	add(http.MethodPost, "/v1/invoiceitems", Claim{Level: "L3", Stateful: true, ScorecardCases: []string{"invoices.one_time_invoice_flow.succeeds"}, Risks: []string{"manual one-time invoice item evidence only; tax, discounts, and pending invoice-item collection are not modeled"}})
	add(http.MethodGet, "/v1/invoiceitems/{id}", Claim{Level: "L2", Stateful: true, Risks: []string{"manual one-time invoice item evidence only"}})
	...
	add(http.MethodGet, "/v1/credit_notes/{id}/lines", Claim{Level: "L2", Stateful: true, Risks: []string{"returns one synthesized line derived from the stored credit note amount and memo/reason; stored line-item breakdowns are not modeled"}})
	...
	add(http.MethodPost, "/v1/payment_intents/{id}", Claim{Level: "L3", Stateful: true, Risks: []string{"metadata merge plus evidenced description only; amount, currency, and status stay immutable"}})
	add(http.MethodGet, "/v1/payment_intents/search", Claim{Level: "L3", Stateful: true, Risks: []string{"supports a measured PaymentIntent search subset for id, customer, status, and metadata equality clauses joined by AND"}})
	...
	add(http.MethodPost, "/v1/payment_methods", Claim{Level: "L2", Stateful: true, WebhookEvents: []string{"payment_method.attached"}, Risks: []string{"creates local sandbox card projection only; real card data is rejected"}})
	add(http.MethodGet, "/v1/payment_methods/{id}", Claim{Level: "L2", Stateful: true, Risks: []string{"retrieves local sandbox card projection only"}})
	add(http.MethodPost, "/v1/payment_methods/{id}", Claim{Level: "L2", Stateful: true, Risks: []string{"metadata/billing_details update is accepted as a compatibility no-op for projected local cards"}})
	add(http.MethodPost, "/v1/payment_methods/{id}/attach", Claim{Level: "L3", Stateful: true, WebhookEvents: []string{"payment_method.attached"}, Risks: []string{"attaches projected sandbox cards to local customer metadata"}})
	add(http.MethodPost, "/v1/payment_methods/{id}/detach", Claim{Level: "L3", Stateful: true, Risks: []string{"detaches projected sandbox cards from local customer metadata"}})
	...
	add(http.MethodGet, "/v1/customers/{customer}/payment_methods/{id}", Claim{Level: "L2", Stateful: true, Risks: []string{"retrieves projected sandbox cards attached to the requested customer"}})
