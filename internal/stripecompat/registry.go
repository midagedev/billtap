...
func DefaultClaims() []Claim {
	...
	add(http.MethodGet, "/v1/disputes", Claim{Level: "L2", Stateful: true, WebhookEvents: []string{"charge.dispute.created", "charge.dispute.closed"}, Risks: []string{"local dispute evidence only; representment workflow is not modeled"}})
	add(http.MethodGet, "/v1/disputes/{id}", Claim{Level: "L2", Stateful: true})
	add(http.MethodPost, "/v1/disputes/{id}", Claim{Level: "L2", Stateful: true})
